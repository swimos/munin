  ...
  private static final Form<Answer> ANSWER_FORM = new Form<>() {
    ...
    public Item mold(Answer object) {
      ...
      if (!object.taxa().isEmpty()) {
        result = result.slot("taxa", swim.munin.filethesebirds.digest.Forms.forSetString().mold(object.taxa()).toValue());
      }
      if (!object.reviewers().isEmpty()) {
        result = result.slot("reviewers", swim.munin.filethesebirds.digest.Forms.forSetString().mold(object.reviewers()).toValue());
      }
      return result;
    }
    ...
    public Answer cast(Item item) {
      ...
      try {
        ...
        if (tag().equals(tag)) {
          ...
          final Set<String> taxaSet = swim.munin.filethesebirds.digest.Forms.forSetString().cast(taxaItem);
          result.addAllTaxa(taxaSet);
          // reviewers
          final Item reviewersItem = item.get("reviewers");
          final Set<String> reviewersSet = swim.munin.filethesebirds.digest.Forms.forSetString().cast(reviewersItem);
